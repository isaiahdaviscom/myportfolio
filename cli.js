...
function printHelp() {
	...
	const cmd     = (name, desc, hint = '') =>
		console.log(`  ${col('green', name.padEnd(26))}${desc}${hint ? C.dim + '  ' + hint + C.reset : ''}`);
...
function runProcess(cmd, args = [], opts = {}) {
	return new Promise((resolve, reject) => {
		const child = spawn(cmd, args, {
			stdio: 'inherit', shell: true, cwd: projectRoot, ...opts,
		});
		child.on('close', code =>
...
async function runServe(withCms = false) {
	if (withCms) {
		...
		const hugo = spawn('hugo', ['server', '-D',
			'--config', 'hugo.toml,config.development.toml'], {
...
async function runNew(type, slug) {
	...
	const filename = type === 'post'
		? `posts/${slug}.md`
		: `portfolio/${slug}.md`;
	console.log(col('cyan', `\n📝 Creating ${hugoType}/${slug}.md ...`));
	await runProcess('hugo', ['new', filename,
		'--config', 'hugo.toml,config.development.toml']);
...
function runStatus() {
	...
	try {
		...
		gitStatus = dirty ? col('yellow', ` (${dirty.split('\n').length} uncommitted)`) : col('green', ' (clean)');
...
(async () => {
	try {
		switch (command) {
			...
			case 'version': console.log(`${pkg.name} v${pkg.version}`); break;
			...
			case 'css': {
				...
				if (!cssMap[sub]) {
					console.error(`❌ Unknown css subcommand "${sub}". Use: audit | analyze | report | unused | optimize | cleanup`);
					process.exit(1);
				}
