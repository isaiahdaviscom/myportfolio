...
class UnusedCSSDetector {
  ...
  async extractDefinedClasses() {
    ...
    console.log(`   Found ${this.stats.classesDefinedInCSS} classes defined in ${this.stats.cssFilesScanned} CSS files`);
  }
  ...
  async extractUsedClasses() {
    ...
    console.log(`   Found ${this.stats.classesUsedInTemplates} classes used in ${this.stats.templateFilesScanned} template files`);
  }
  ...
  async generateUnusedClassesJSON() {
    const unusedData = {
      ...
      summary: {
        ...
        unusedPercentage: ((this.stats.unusedClasses / this.stats.classesDefinedInCSS) * 100).toFixed(2)
      },
  ...
  createUnusedClassesReport() {
    const unusedPercentage = ((this.stats.unusedClasses / this.stats.classesDefinedInCSS) * 100).toFixed(2);
...
${this.cssFileAnalysis.map(file => 
  `### ${file.file}
- **Classes Defined**: ${file.classes}
- **Sample Classes**: ${file.classList.slice(0, 10).map(cls => `\`${cls}\``).join(', ')}${file.classList.length > 10 ? '...' : ''}`
).join('\n\n')}

## Unused Classes by Category

### Custom Classes
${unusedArray.filter(cls => !/^(sm:|md:|lg:|xl:|2xl:|hover:|focus:|active:|group-)/.test(cls) && 
  !/^(text-|bg-|p-|m-|w-|h-|flex|grid|border)/.test(cls))
  .slice(0, 20).map(cls => `- \`${cls}\``).join('\n')}

### Utility Classes
${unusedArray.filter(cls => /^(text-|bg-|p-|m-|w-|h-|flex|grid|border)/.test(cls))
  .slice(0, 20).map(cls => `- \`${cls}\``).join('\n')}

### Responsive Classes
${unusedArray.filter(cls => /^(sm:|md:|lg:|xl:|2xl:)/.test(cls))
  .slice(0, 10).map(cls => `- \`${cls}\``).join('\n')}

### State Classes
${unusedArray.filter(cls => /^(hover:|focus:|active:|group-)/.test(cls))
  .slice(0, 10).map(cls => `- \`${cls}\``).join('\n')}
...
`;
  ...
  async generateCleanupSuggestions() {
    const suggestions = {
      highConfidence: Array.from(this.unusedClasses).filter(cls => 
        !cls.includes('hover') && 
        !cls.includes('focus') && 
        !cls.includes('active') &&
        !cls.includes('sm:') &&
      ...
      mediumRisk: Array.from(this.unusedClasses).filter(cls => 
        cls.includes('hover') || 
        cls.includes('focus') || 
        cls.includes('active') ||
        cls.includes('sm:') ||
      ...
      preserveClasses: Array.from(this.unusedClasses).filter(cls => 
        this.shouldPreserveClass(cls)
      )
    };
